...
import 'package:timetable/feature/presentation/widgets/bell_card.dart';
...
class BellsScreen extends StatelessWidget {
  const BellsScreen({Key? key}) : super(key: key);
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          NavigationActions.instance
              .showBellDetailsScreen(SchoolBellModel.emptyBell());
        },
        child: const Icon(Icons.add, color: AppColors.lightTextColor),
      ),
      body: BlocConsumer<BellsCubit, BellState>(
  ...
  Widget _bodyWidget(BuildContext context, List<SchoolBellEntity>? data) {
    if (data == null || data.isEmpty) {
      return const EmptyList(
        labelText: 'Немає розкладу дзвінків',
      );
    } else {
          ...
          itemBuilder: (context, index) {
            return Dismissible(
              ...
              onDismissed: (direction) async {
                await context.read<BellsCubit>().delBell(data[index]);
              },
              ...
              child: GestureDetector(
                onTap: () {
                  NavigationActions.instance.showBellDetailsScreen(data[index]);
                },
                child: BellCard(data: data[index]),
              ),
            );
          });
    }
